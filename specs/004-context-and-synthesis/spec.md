...
- FR-5: Support model selection: Gemma 4 2B via Ollama (default, localhost:11434) or GPT-4o-mini (opt-in, requires `OPENAI_API_KEY`). Model specified by user in query request.
...
- NFR-5: If Ollama is unavailable, return a `503 Service Unavailable` error. There is **no automatic fallback** to GPT-4o-mini — this would violate the privacy guarantee. Users who want GPT-4o-mini must explicitly opt in.
...
## Failure Cases
- **LLM timeout** (Ollama or GPT-4o-mini): Send SSE `error` event with `"LLM response timed out"`. Close stream.
- **LLM error** (Ollama connection refused, OpenAI 5xx): Send SSE `error` event with `"LLM service temporarily unavailable"`. Close stream.
- **Ollama unavailable** (default LLM): Return `503 Service Unavailable`. No automatic fallback to GPT-4o-mini.
...
- Ollama unavailability returns clear 503 error without silent fallback.
