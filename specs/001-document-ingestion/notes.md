...
- [tiktoken library](https://github.com/openai/tiktoken) — Token counting with cl100k_base encoding
...
We chose NOT to use LangChain's implementation directly to avoid the large dependency. Instead, implement the same algorithm (~50 lines) with tiktoken for token counting.
...
The `chunks` table includes an `embedding VECTOR(1536)` column that is created as nullable. This spec creates chunks with NULL embeddings. Spec 002-embedding-and-storage is responsible for populating embeddings and creating the HNSW index. This separation allows the ingestion pipeline to complete even if the embedding API is temporarily unavailable.
